#
#   Crawler routines for gmca, derived from P11
...
def scan_data(data_dir):
    ...
    run_dirs = [d for d in run_dirs if d.startswith('run') and os.path.isdir(os.path.join(data_dir,d))]
    ...
    # Create the result
    #run_list = [int(r.replace("run", "")) for r in run_list]
    result = {
        'run': run_list,
        'status': status
    }
